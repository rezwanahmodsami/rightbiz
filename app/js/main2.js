...
$(document).on('keyup','.search-inp',function(e){
    ...
    if(str=='' || code==27 || code==13){
        ...
        if(!$inArray(code, ignore_codes)){
            ...
            ths.append('after', '<span class="absolute text-large text-lightgrey clear_input" style="cursor:pointer;" onclick="'+(ths.attr('data-type')=='sector' ? 'clear_biz_sector(this)':'clear_biz_location(this)')+'">&times;</span>');
...
function clear_biz_sector(el){
    $('#search-sector').val('');
    $('.search-sector .stype').val('');
    $(el).remove();
    close_suggest();
}

function clear_biz_location(el){
    $('#search-location').val('');
    $('.search-location .stype').val('');
    $(el).remove();
    close_suggest();
}
